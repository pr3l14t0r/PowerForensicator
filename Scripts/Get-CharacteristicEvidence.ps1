function Get-CharacteristicEvidence
{
    [CmdletBinding()]
    param (
        [Parameter()]
        ...
        [ValidateNotNullOrEmpty()]
        [string]
        $OutFileName = [System.IO.Path]::Combine($ImageDirectory,([System.String]::Concat($VM,"-",$ActionName,"-","CharacteristicEvidence.txt"))), #todo: add explanation
...
$Component = "Get-CharacteristicEvidence"
Write-VBox4PwshLog -Message ("Fetching the NOISE evidence file(s)") -VM $VM -Component $Component -Level Verbose
$NoiseEvidencePaths = Get-ChildItem -Path $ImageDirectory -Recurse -Force -Filter ("*"+$VM+"*NOISE*Evidence*")
if($null -eq $NoiseEvidencePaths)
{
    Write-VBox4PwshLog -Message ("No files found! That does usually mean, that there are no differences in FSMounts between INIT and NOISE!") -VM $VM -Component $Component -Level Verbose
}
else
{
...
$ActionEvidencePaths = Get-ChildItem -Path $ImageDirectory -Recurse -Force -Filter ("*"+$VM+"*"+$ActionName+"*Evidence*")
if($null -eq $ActionEvidencePaths)
{
    Write-VBox4PwshLog -Message ("No files found! That does usually mean, that there are no differences in FSMounts between ACTION and NOISE!") -VM $VM -Component $Component -Level Verbose
}
else
{
...
if($null -eq $characteristicNoiseContent)
{
    ...
    [string[]]$characteristicNoiseContent = @()
}
...
if($null -ne $ActionEvidencePaths)
{
    ...
    {
        if($characteristicNoiseContent.Name.Contains($line.Name))
        {
...
Write-VBox4PwshLog -Message ("Found <"+$CharacteristicEvidences.Count+"> chracteristic evidences for action <"+$ActionName+"> on vm <"+$VM+">!") -VM $VM -Component $Component
